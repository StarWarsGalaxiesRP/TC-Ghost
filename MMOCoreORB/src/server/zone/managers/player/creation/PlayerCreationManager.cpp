					...
					try {
						...
						if (res != NULL && res->next()) {
							...
							if (timeVal.miliDifference() < 600000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character every 10 minutes. Repeat attempts prior to 10 minutes elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
